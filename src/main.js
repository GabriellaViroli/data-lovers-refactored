...
function makeCardInfo(data, cardContent, cardTitle) {
    let dataInfos = '';
    ...
    cardContent.innerHTML = "<ul>" + dataInfos + "</ul>";
...
housesGroupBtn.addEventListener('click', listenedElement => {
    if (listenedElement.target.classList.contains('modal-btn')) {
        const house = listenedElement.target.dataset.house;
...
charactersContent.addEventListener('click', (listedCharacter) => {
    if (listedCharacter.target.classList.contains('list-elements')) {
        ...
        makeCardInfo(filterCharacters[0], cardContent, cardTitle);
    }
...
function displayBooksList() {
    ...
    modalContent.addEventListener('click', listenedElement => {
        if (listenedElement.target.classList.contains('list-elements')) {
